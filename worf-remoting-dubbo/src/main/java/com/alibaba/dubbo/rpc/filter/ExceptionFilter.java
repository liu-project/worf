        ...
        try {
            ...
            if (result.hasException() && GenericService.class != invoker.getInterface()) {
                try {
                    ...
                    // FastRuntime exception子类的异常转换成父类抛出，否则会引起另一个项目找不到其子类异常导致序列化失败
                    if (exception instanceof FastRuntimeException) {
                    	FastRuntimeException fe = (FastRuntimeException)exception;
                    	return new RpcResult(new FastRuntimeException(fe.getCode(), fe.getMessage(), fe.getCause()));
                    }
                    // 在方法签名上有声明，直接抛出
                    try {
